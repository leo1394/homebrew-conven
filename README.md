...
`loom` is a focused launcher for local microservice development. It starts only
...
```text
Looming local services: user-svc, order-svc, payment-svc
```

Loom itself is written in Go, but the services it launches are not limited to
Go. Each service declares prepare, build, and run commands as argv arrays in the
workspace manifest, so Loom can run Go, Java, Node.js, or any other service that
...
Before the GitHub repository is published, run the source build from the root
of a Loom checkout:
...
After the repository is published at `leo1394/homebrew-loom`, install the HEAD
Formula with:
...
Homebrew core also contains a cask named `loom`, so use the fully qualified
Formula name to avoid installing the unrelated cask.
...
Loom separates reusable mechanics from project-specific conventions:
...
Initialize the current directory as a Loom workspace, inspect the repositories
...
loom doctor
loom services --start --dry-run user-svc order-svc payment-svc
loom services --start user-svc order-svc payment-svc
```

The service names above are from the fallback example. When repositories are
detected, pass names shown by `loom services --list` instead.

On first initialization, `loom init` scans only immediate child directories that
...
instead of being overwritten. Use `loom services --registry` after the set of
child repositories changes. Before first runtime use, Loom adds `/runtime/` to
...
7. Use `loom policy --edit` to declare project rules scanning cannot infer, or
   import a complete candidate from a project-local generator with
   `loom policy --import <yaml-file> --edit`. Then run `doctor`, a start dry-run,
...
```bash
loom services --start --tail user-svc order-svc
```

To inspect the resolved plan without starting anything:

```bash
loom services --start --dry-run user-svc order-svc
```
...
```bash
loom policy --edit
loom policy --import ./generated-loom.yaml
loom policy --import ./generated-loom.yaml --edit
loom policy --reset
```
...
Loom publishes the draft's exact bytes only after the editor exits successfully,
...
invalid YAML, unknown field, or detected concurrent edit, Loom does not publish
the draft or overwrite the concurrently observed manifest. A rejected changed
draft is kept with mode `0600` under `.loom/backups/`, which Loom adds to
...
serializes cooperating Loom writers. An arbitrary external writer does not have
...
`--import <yaml-file>` reads a complete local Loom v1 manifest and publishes its
...
under `.loom/backups/`; byte-identical content is a no-op. With `--edit`, Loom
...
Successful import validation proves that the file is a valid Loom v1 manifest;
...
follow an import with `loom doctor` and
`loom services --start --dry-run SERVICE...` before a real start.
...
Before replacing an existing manifest, Loom saves its exact bytes with mode
...
loom policy --edit
loom doctor
loom services --start --dry-run SERVICE...
```

| Command | Purpose | Treatment of manual rules |
| --- | --- | --- |
| `loom init` | Create a missing manifest | Never overwrites an existing manifest |
| `loom policy --edit` | Edit a validated temporary draft | Preserves everything the user does not change |
| `loom policy --import <yaml-file> [--edit]` | Publish a complete local v1 manifest, optionally after editing a private draft | Replaces the whole manifest; backs up but never merges or modifies the source |
| `loom services --registry` | Conservatively merge newly scanned facts | Existing non-empty manual fields take precedence |
| `loom policy --reset` | Rebuild the whole manifest from scan facts | Discards manual declarations; restore them from the printed backup or edit again |

### Generated and manually confirmed fields

v1 has only the central `.loom/loom.yaml`; Loom does not create or read distributed
...
| Kubeconfig/secrets | Not generated | Must not hard-code them | Configure through `loom config`, environment, or an external credential system |
...
`loom services --registry` afterward to conservatively backfill empty analyzer
...
Loom only backfills a missing `kind` or the entire missing `discovery` metadata.
...
repository being discovered means only that Loom can derive its entry point and
...
When `loom services --start` receives no service arguments, it opens the built-in
...
`loom services --restart` without service arguments examines every service
...
```bash
loom services --restart
loom services --restart user-svc order-svc
loom services --restart --tail user-svc
```
...
build steps for all targets finish before Loom stops any target. Loom also
...
untrusted saved process remains, Loom safely resets
...
its `runner.run` references that default artifact, Loom fails before opening a
...
stopped, Loom clears the session and releases its connection lease while
...
`loom services --status` displays the saved PID and PGID. After confirming that
the PGID belongs to the Loom session, use
`loom services --stop --force SERVICE` for one service or
`loom services --stop --all --force` for the full session. `--force` bypasses
identity verification and signals the saved PGID directly; use it only for a
manually verified recovery.

`loom services --stop-all` is the exact shorthand for
`loom services --stop --all`: both use the same service cleanup and connection
...
`Managed=false` is not owned by Loom. Both forms remove only the current session
reference and never terminate that external connection.

When a workspace has no session, `loom services --status` lists shared
connection records from the current user's effective Loom state root, including
fingerprint, PID/PGID, and effective lease count. After confirming the target,
`loom services --stop --all --force` examines those records and force-removes only
...
This path recovers connection process groups and records left behind after Loom
...
Loom stores user-wide settings in `~/.loom/config` and workspace settings in
...
loom config --list
loom config ktctl.path
loom config ktctl.path /opt/homebrew/bin/ktctl
loom config ktctl.kubeconfig /secure/dev-kubeconfig
loom config --unset ktctl.path
loom config --unset ktctl.kubeconfig

# User-wide scope; also works outside a workspace.
loom config --global --list
loom config --global ktctl.path '~/bin/ktctl'
loom config --global ktctl.kubeconfig '~/.kube/dev-config'
loom config --global --unset ktctl.path
loom config --global --unset ktctl.kubeconfig
```
...
The ktctl runtime settings consumed by Loom are `ktctl.path` and
...
the current directory, Loom walks upward and stops at the nearest `.loom`
directory. That directory is a hard workspace boundary: if it does not contain
`loom.yaml`, Loom reports an incomplete workspace and does not continue to a
parent workspace. If no `.loom` directory is found, the current directory is
outside a Loom workspace. The user-wide `~/.loom` directory is reserved for
global settings and is never a workspace boundary. `loom init` therefore
...
```bash
(cd /path/to/workspace && loom services --status)
```
...
by discovery update only this file. `loom init` may also merge `/runtime/` into
the central `.loom/.gitignore`, but it never writes Loom configuration or
runtime copies into child service repositories.

`loom policy` also operates on this one canonical file; Loom never creates or
...
Loom injects the resolved absolute workspace root into every local service as
`LOOM_WORKSPACE`, replacing any inherited value. This is read-only metadata for
the service process; the Loom CLI never reads it to discover or select a
workspace.
...
the same manifest. Use `loom policy --edit` for validated manual changes and
...
and can publish a separate runtime bootstrap. Loom applies shared policy patches,
...
`loom policy --import <yaml-file> --edit`, and commit the resulting canonical
`.loom/loom.yaml`. Machine-local kubeconfig paths can be set with
`loom config ktctl.kubeconfig <path>`; credentials should stay in environment or
...
Loom checks that it is a directory after prepare/build and immediately before
...
Local dependency graphs may contain cycles. Loom first groups each cycle into a
...
TCP `readiness` endpoint. If all endpoints are already reachable, Loom reuses
the existing network path instead of starting another connection process.

Connections started by Loom use a current-user global lock, persistent records,
and workspace leases. When multiple workspaces reuse one managed ktctl
connection, releasing one workspace does not interrupt the others. Loom stops
the connection only after the final lease is released. If networking is already
provided by an external process, Loom reuses reachability without taking
ownership. Set `connection.sudo: true` when a custom connection must run as
root. Loom first runs interactive `sudo -v`, starts through `sudo -n`, and tracks
the actual connection descendant rather than only the outer sudo process. If
the sudo timestamp has expired at shutdown, Loom requests authorization again.
...
Loom supports two explicit routing contracts:

1. With the legacy environment contract, Loom injects dependency `localEnv` or
...
routing contract, otherwise Loom rejects the ambiguous plan before startup.
...
For `connection.driver: ktctl`, Loom selects the executable in this order:
...
```bash
loom config ktctl.path /absolute/path/to/ktctl
# Or set a default for every workspace:
loom config --global ktctl.path ktctl-custom
```
...
launch, Loom resolves a PATH command to its executable path; this keeps
...
LOOM_KUBECONFIG=/secure/dev-kubeconfig \
  loom doctor --env dev --context dev-cluster --namespace dev

# Requires an environments.test profile in the manifest.
KTCTL_KUBECONFIG=/secure/test-kubeconfig loom doctor --test

loom config ktctl.kubeconfig /secure/dev-kubeconfig

loom services --start --dev \
  --context dev-cluster \
  --namespace dev \
  user-svc order-svc
```
...
`sudo -n <resolved-ktctl> --kubeconfig <file> ... connect`; Loom performs
...
v1 requires the resolved kubeconfig to be a single file. Loom rejects a
...
connection log. Loom truncates `connection.log` only when it actually starts a
...
Before resetting `current`, Loom verifies saved PID/PGID and process identity.
...
lock, and log files use user-only file permissions. Loom rejects symlinks,
...
restart. Loom does not create historical run directories: restart reuses
...
as an unknown field. Loom does not discover, migrate, or delete workspace
...
Loom user state root selected, in order, by `LOOM_STATE_HOME`,
