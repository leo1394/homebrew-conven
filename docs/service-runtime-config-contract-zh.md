...
注册中心读取遇到临时超时、EOF、连接中断或 HTTP 429/502/503/504 时，最多尝试
4 次，依次退避 250ms、500ms、1s；单次请求仍限时 5 秒。恢复后重新完成整个观察
窗口，并始终与启动前的原始快照比较，失败请求不计入隔离证明。观察总时限为配置的
观察时长加 30 秒。权限错误、无效响应和新增实例仍然失败关闭；持续不可达时回滚
本次启动，不删除注册中心数据。
