...
if curl -fsL --retry 3 --retry-delay 1 --connect-timeout 10 \
  ...
  -o "$CHECKSUM_FILE" 2>/dev/null; then
  ...
  if ! grep -Fq "/refs/tags/$TAG.tar.gz\"" "$FORMULA"; then
...
if ! "$EXECUTABLE" --version | grep -Fq "conven version $VERSION "; then
