...
func InitWorkspaceDetails(cwd string, application []byte) (InitResult, error) {
	...
	if sameDirectory(workspace, resolvedUserHome()) {
		return result, fmt.Errorf("cannot initialize a Loom workspace in the user home directory %q: ~/.loom is reserved for global configuration", workspace)
	}
	boundary := filepath.Join(workspace, ".loom")
	info, err := os.Lstat(boundary)
	if err == nil && (info.Mode()&os.ModeSymlink != 0 || !info.IsDir()) {
		return result, fmt.Errorf("loom workspace boundary %q is not a directory; symbolic links are not allowed", boundary)
	}
	if err != nil && !os.IsNotExist(err) {
		return result, fmt.Errorf("inspect loom workspace boundary %q: %w", boundary, err)
	}
	...
	if err == nil {
		if info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() {
			return result, fmt.Errorf("loom manifest %q must be a regular file, not a symbolic link", manifest)
		}
	...
	if !os.IsNotExist(err) {
		return result, fmt.Errorf("inspect loom manifest %q: %w", manifest, err)
	}
	...
	if err := os.MkdirAll(boundary, 0700); err != nil {
		return result, fmt.Errorf("create loom workspace boundary %q: %w", boundary, err)
	}
	if err := ensureRuntimeIgnored(boundary); err != nil {
		return result, err
	}
...
func ensureLoomPathIgnored(boundary string, rule string) error {
	...
	if err == nil {
		...
		return fmt.Errorf("inspect loom gitignore %q: %w", path, err)
	}
	var data []byte
	if err == nil {
		data, err = os.ReadFile(path)
		if err != nil {
			return fmt.Errorf("read loom gitignore %q: %w", path, err)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("open loom gitignore %q: %w", path, err)
	}
	if _, err := file.WriteString(prefix + rule + "\n"); err != nil {
		file.Close()
		return fmt.Errorf("update loom gitignore %q: %w", path, err)
	}
	if err := file.Close(); err != nil {
		return fmt.Errorf("close loom gitignore %q: %w", path, err)
	}
	return nil
}
...
func publishNewManifest(path string, data []byte) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("create temporary loom manifest: %w", err)
	}
	temporaryName := temporary.Name()
	defer os.Remove(temporaryName)
	if err := temporary.Chmod(0600); err != nil {
		temporary.Close()
		return false, fmt.Errorf("protect temporary loom manifest: %w", err)
	}
	if _, err := temporary.Write(data); err != nil {
		temporary.Close()
		return false, fmt.Errorf("write temporary loom manifest: %w", err)
	}
	if err := temporary.Sync(); err != nil {
		temporary.Close()
		return false, fmt.Errorf("sync temporary loom manifest: %w", err)
	}
	if err := temporary.Close(); err != nil {
		return false, fmt.Errorf("close temporary loom manifest: %w", err)
	}
	if err := os.Link(temporaryName, path); err != nil {
		if os.IsExist(err) {
			return false, nil
		}
		return false, fmt.Errorf("publish loom manifest %q: %w", path, err)
	}
